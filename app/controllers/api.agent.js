...
let _t = {
    ...
    login: {
        ...
        async post(request, reply) {
            ...
            const status = 1
            const agent = await service_agent.oneByAccountPassword(account, password, status)
            if (agent == null) {
                return { flag: '账号或密码错误！' }
            }
            const user_agent = request.headers['user-agent']
            const ip = request.headers['cf-connecting-ip'] || request.headers['x-real-ip'] || request.ip
            await service_login_log.addLoginLog(agent.id, service_login_log.UserType.AGENT, user_agent, ip)
    ...
    wallet: {
        ...
        async get(request, reply) {
            ...
            const type = query.type
            let res = await service_system_wallet_address.list(address, type, start, size)
            let list = res.list
            let total = res.total
            for (let i = 0; i < list.length; i++) {
                let item = list[i]
