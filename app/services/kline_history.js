...
async function insert(symbol, period, open, close, high, low, vol, ts) {
  ...
  const res = await db.Query(
    "INSERT INTO kline_history(`symbol`,`period`,`open`,`close`,`high`,`low`,`vol`,`ts`,`create_datetime`,`update_datetime`) VALUES(?,?,?,?,?,?,?,?,?,?)",
    [
      symbol,
      period,
      open,
...
async function one(symbol, period, ts) {
  const res = await db.Query(
    "SELECT * FROM kline_history WHERE symbol=? AND period=? AND ts=? LIMIT 1",
    [symbol, period, ts],
  );
...
async function saveKlineArr(symbol, period, klineArr) {
  //   console.log("saveKlineArr============>", symbol, period, klineArr.length);
  let arr = [];
  for (let i = 0; i < klineArr.length; i++) {
    let item = klineArr[i];
    const create_datetime = utils99.Time(config.web.timezone);
    const update_datetime = utils99.Time(config.web.timezone);
    let timestamp = utils99
      .moment(item.timestamp)
      .format("YYYY-MM-DD HH:mm:ss");
    let v = `('${symbol}', '${period}', '${round(item.open, 6)}', '${round(item.close, 6)}', '${round(item.high, 6)}', '${round(item.low, 6)}', '${round(item.volume, 6)}', '${timestamp}', '${create_datetime}', '${update_datetime}')`;
    arr.push(v);
  }
  res = await update2(arr);
  //   console.log(res);
}
async function update2(arr) {
...
function klineDataDemo01(symbol, period, item, time_step) {
  ...
  let close = Number(
    isUp
      ? open + random(open / 10000, open / 1000)
      : open - random(open / 10000, open / 1000),
  );
  // close = round(close, 8)
  ...
  for (let i = 0; i < DATE_LENGTH; i++) {
    ...
    if (isUp) {
      ...
      if (HIGH < temp) {
        ...
        low =
          Math.min(open, close) -
          (open * random(close / 1000, close / 100)) / 100;
        if (low < LOW) low = LOW;
      } else {
        close = temp;
        high =
...
function oneRandomList(arr, path_dir) {
  ...
  file_name = path_dir + "/up_20260410_222740.json";
  console.log("指定Kline模板文件：", file_name);
...
function templateToKline(tempalteArr, dayItem) {
  ...
  //   console.log("dayItem.timestamp=========------>>>", dayItem.timestamp);
  //   let date = utils99.moment(dayItem.timestamp).format("YYYY-MM-DD");
  let date = utils99
    .moment(dayItem.timestamp, "YYYY/MM/DD HH:mm:ss")
    .format("YYYY-MM-DD");
...
let _t = {
  ...
  async listBySymbol(symbol, period, size, isCurrTime = true) {
    ...
    if (isCurrTime) {
      ...
      let res = db.Query(
        "SELECT " +
          f +
          " FROM kline_history WHERE symbol=? AND period=? AND ts<=? ORDER BY ts DESC LIMIT ?",
        [symbol, period, ts, size],
      );
      return res;
    }
    let res = db.Query(
      "SELECT " +
        f +
        " FROM kline_history WHERE symbol=? AND period=? ORDER BY ts DESC LIMIT ?",
      [symbol, period, size],
    );
    return res;
  },
  ...
  async update(symbol, period, open, close, high, low, vol, ts) {
    ...
    const updateRes = await db.Query(
      "UPDATE kline_history SET `symbol`=?,`period`=?,`open`=?,`close`=?,`high`=?,`low`=?,`vol`=?,`ts`=?,`update_datetime`=? WHERE `symbol`=? AND `period`=? AND `ts`=?",
      [
        symbol,
        period,
        open,
  ...
  async updateNameByName(old_name, new_name) {
    const res = await db.Query(
      "UPDATE kline_history SET `symbol`=? WHERE `symbol`=?",
      [new_name, old_name],
    );
    return res;
  },
  ...
  async clearHistoryBySymbol(symbol) {
    const res = await db.Query("DELETE FROM kline_history WHERE symbol=?", [
      symbol,
    ]);
    return res;
  },
